...
int 	CmdStatSockConnect()	//setup local server, from address initializaion to listen
{
	...
	g_sktListen = socket(AF_INET, SOCK_STREAM, 0) ;
	...
	bzero((char*)&SockRemoteAdrs, nAdrsLen) ;
        printf("g_sktCmdAck and g_sktStaInfo wait for connection.\n") ;
...
{
	struct sockaddr_in SockRemoteAdrs;
        memset(&SockRemoteAdrs, 0, sizeof(SockRemoteAdrs));
	...
	if (connect(g_sktData, (struct sockaddr *)&SockRemoteAdrs,sizeof(struct sockaddr)) == -1)
	{
        ...
        DEBUG_OUTPUT(("ROS IP = %s, PORT = %d.\n",inet_ntoa(SockRemoteAdrs.sin_addr), nDataPort)) ;
	...
	while(nSndLenTmp)
	{
		...
		{  
			...
			case 0:  
				...
				if(nSelTimer==0)
				{
					logMsg("error in fw_tcpfunc.cpp,SelectSend(),sockfd time out 0.5 second\n", 0, 0, 0, 0, 0, 0);
				}
				else
				{
					logMsg("error in fw_tcpfunc.cpp,SelectSend(),sockfd time out 1 second\n", 0, 0, 0, 0, 0, 0);
				}
				nSelTimer++;
				taskDelay(60);
				break;
